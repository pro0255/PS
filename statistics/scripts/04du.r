...
selector_du4 = c("pozadavek_5", "vyrobce")

selector_du4


data_du4 = s_data[, selector_du4]

data_du4$pozadavek_5 = factor(data_du4$pozadavek_5, levels=c(true_dich_value, false_dich_value))

kont_tab = table(data_du4$vyrobce, data_du4$pozadavek_5)
kont_tab
...
kont_tab_rel = round(100*prop.table(kont_tab, margin = 1), 0)
kont_tab_rel
...
l = length(data_du4$pozadavek_5) - 1 
...
muj_mos = 
  ggplot(data_du4)+
  geom_mosaic(aes(x = product(pozadavek_5, vyrobce),
                  fill = pozadavek_5),
              offset = 0.02,
              colour = "black")+  
  labs(x = "", y = "",  title = "")+
...
tmp = ggplot_build(muj_mos)

muj_mos + 
  geom_text(data = tmp$data[[1]], 
...
moje_tab = matrix(c(59,13,24,47), 
                  byrow = T, 
                  ncol = 2)
rownames(moje_tab) = c("Amber", "Dim")
colnames(moje_tab) = c("Ne", "Ano")
moje_tab


library(epiR)
epi.2by2(moje_tab, conf.level = 0.95)
